function [count0 count1 count2 count3]= exa0() 
	...
	for beta = -30:1:0
		...
		V2=lmivar(3,[1 4;4 5]);			% V2=[X4 X2;X2 X3]
		E2=lmivar(3,[6 7;8 9]);			% E2
	...
	% for beta = -30:0.5:0
		...
		% E2=lmivar(3,[7 8;9 10]);		% E2
				
		% E1=lmivar(1,[2 1]);				% E1
		 % W=lmivar(1,[2 1]);				% W
				
		% lmiterm([1 1 1 E1],1,1);		% E1<0:E1
		
		% lmiterm([-2 1 1 W],1,1);		% 0<W:W
		
		% lmiterm([3 1 1 V1],4,A1,'s');		% 4*V1*A1+4*A1'*V1
		% lmiterm([3 1 1 E1],TNegA1,A1,'s');	% -A1'*E1*A1-A1'*E1*A1
		% lmiterm([3 1 1 E2],TNegA1,2,'s');	% -2E2'*A1-2A1'*E2
		% lmiterm([3 1 1  W],1,1);			% W
		% lmiterm([3 1 2 -E2],1,1);			% E2'
		% lmiterm([3 2 1 E2],1,1);			% E2
		% lmiterm([3 2 2 E1],0.5,1);			% E1
		
		% lmiterm([4 1 1 V1],4,A2,'s');		% 4*V1*A2+4*A2'*V1
		% lmiterm([4 1 1 E1],TNegA1,A2,'s');	% -A1'*E1*A2-A2'*E1*A1
		% lmiterm([4 1 1 E2],TNegA1,2,'s');	% -2E2'*A1-2A1'*E2
		% lmiterm([4 1 1  W],-1,1);			% -W
		% lmiterm([4 1 2 -E2],1,1);			% E2'
		% lmiterm([4 2 1 E2],1,1);			% E2
		% lmiterm([4 2 2 E1],0.5,1);			% E1
		
		% lmiterm([5 1 1 V2],4,A1,'s');		% 4*V2*A1+4*A1'*V2
		% lmiterm([5 1 1 E1],TNegA2,A1,'s');	% -A2'*E1*A1-A1'*E1*A2
		% lmiterm([5 1 1 E2],TNegA2,2,'s');	% -E2'*A2-A2'*E2
		% lmiterm([5 1 1  W],-1,1);			% -W
		% lmiterm([5 1 2 -E2],1,1);			% E2'
		% lmiterm([5 2 1 E2],1,1);			% E2
		% lmiterm([5 2 2 E1],0.5,1);			% E1
		
		% lmiterm([6 1 1 V2],4,A2,'s');		% 4*V2*A2+4*A2'*V2
		% lmiterm([6 1 1 E1],TNegA2,A2,'s');	% -A2'*E1*A2-A2'*E1*A2
		% lmiterm([6 1 1 E2],TNegA2,2,'s');	% -2E2'*A2-2A2'*E2
		% lmiterm([6 1 1  W],1,1);			% W
		% lmiterm([6 1 2 -E2],1,1);			% E2'
		% lmiterm([6 2 1 E2],1,1);			% E2
		% lmiterm([6 2 2 E1],0.5,1);			% E1
	...
	% for beta = -30:1:0
		% for alpha =  -70:1:10
			...
			% lmiterm([-1 1 1 W],1,1);		% 0<W:W

			% lmiterm([2 1 1 V1],1,A1,'s');	% V1*A1+A1'*V1'+W<0:V1*A1+A1*V1'
			% lmiterm([2 1 1 W],1,1);			% V1*A1+A1*V1'+W<0:W

			% lmiterm([3 1 1 V2],1,A2,'s');	% V2*A2+A2'*V2'+W<0:V2*A2+A2*V2'
			% lmiterm([3 1 1 W],1,1);			% V2*A2+A2*V2'+W<0:W

			% lmiterm([4 1 1 V1],1,A2,'s');	% V1*A2+A2'*V1'-W<0:V1*A2+A2'*V2'
			% lmiterm([4 1 1 W],-1,1);		% V1*A2+A2'*V1'-W<0:-W

			% lmiterm([5 1 1 V2],1,A1,'s');	% V2*A1+A1'*V2'-W<0:V2*A1+A1'*V2'
			% lmiterm([5 1 1 W],-1,1);		% V2*A1+A1'*V2'-W<0:-W
	...
	% for beta = -30:1:0
		% for alpha =  -70:1:10
			...
			% if tmin<0
				% count2 = count2 + 1;
				% plot(alpha,beta,'o','markersize',6);
				% hold on;
			% end
		% end
	% end
	...
	% for beta = -30:1:0
		% for alpha =  -70:1:10
			...
			% V1=lmivar(3,[1 2;3 4]);			% V1=[X1 X2;X3 X4]
			% V2=lmivar(3,[5 2;3 4]);			% V2=[X5 X2;X3 X4]
			 % W=lmivar(3,[6 7;8 9]);			% W

			% lmiterm([-1 1 1 V1],1/2,1,'s');	% 0<V1:V1
			
			% lmiterm([-2 1 1 V2],1/2,1,'s');	% 0<V2:V2 
						
			% lmiterm([-3 1 1  W],1/2,1,'s');	% 0<W:W

			
			% lmiterm([4 1 1 V1],1,A1,'s');	% V1*A1+A1'*V1'+W<0:V1*A1+A1*V1'
			% lmiterm([4 1 1 W],1/2,1,'s');			% V1*A1+A1*V1'+W<0:W

			% lmiterm([5 1 1 V2],1,A2,'s');	% V2*A2+A2'*V2'+W<0:V2*A2+A2*V2'
			% lmiterm([5 1 1 W],1/2,1,'s');			% V2*A2+A2*V2'+W<0:W

			% lmiterm([6 1 1 V1],1,A2,'s');	% V1*A2+A2'*V1'-W<0:V1*A2+A2'*V2'
			% lmiterm([6 1 1 W],-1/2,1,'s');		% V1*A2+A2'*V1'-W<0:-W

			% lmiterm([7 1 1 V2],1,A1,'s');	% V2*A1+A1'*V2'-W<0:V2*A1+A1'*V2'
			% lmiterm([7 1 1 W],-1/2,1,'s');		% V2*A1+A1'*V2'-W<0:-W
